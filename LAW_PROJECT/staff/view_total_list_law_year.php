...
if (isset($_GET['user_id'])) {
    ...
    // Fetch total cases and total days for the year
    $queryTotal = "SELECT COUNT(*) AS yearly_cases, SUM(num_days) AS total_year FROM form_project WHERE user_id = ? AND YEAR(fromDate) = ?";
    $stmtTotal = $conn->prepare($queryTotal);
    $stmtTotal->bind_param("si", $user_id, $year);
    $stmtTotal->execute();
    ...
    // Fetch all cases for the user for the current year
    $queryCases = "SELECT * FROM form_project WHERE user_id = ? AND YEAR(fromDate) = ?";
    $stmtCases = $conn->prepare($queryCases);
    $stmtCases->bind_param("si", $user_id, $year);
    $stmtCases->execute();
